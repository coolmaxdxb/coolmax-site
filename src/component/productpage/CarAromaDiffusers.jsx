...
// --- CORRECTED IMAGE IMPORTS BASED ON YOUR DIRECTORY ---
import DriveMistHero from "../../asset/productimg/CMwebpimg/DriveScent/1.webp"; 
import DriveMistProduct from "../../asset/productimg/CMwebpimg/DriveScent/2.webp"; 
import VentoCarProduct from "../../asset/productimg/CMwebpimg/VentoCar/1.webp"; 
import DeskScentProduct from "../../asset/productimg/CMwebpimg/DeskScent/7.webp"; 
      ...
      <div className="relative z-10 max-w-[1600px] mx-auto px-6 md:px-16">
        ...
        <section className="min-h-[70vh] flex flex-col lg:flex-row items-center pt-24 lg:pt-10 gap-10 text-left">
          ...
          <div className="lg:w-1/2 flex flex-col items-start z-20">
            ...
            >
              <div className="inline-flex items-center gap-3 px-4 py-2 rounded-full bg-slate-100 border border-slate-200 mb-6 shadow-sm mt-16 text-left">
        ...
        <section id="collection" className="py-16 border-t border-slate-100 text-left">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            {[
              ...
              { img: NovaCarProduct, name: "NovaCar – Modern Vehicle Aroma Diffuser", path: "/car-aroma-diffurers/nova-car-diffuser/", desc: "A compact and efficient diffuser designed for shared and frequently used vehicles. Provides reliable fragrance diffusion with minimal attention.", suits: "Fleet vehicles • Service cars • Commercial use" },
            ].map((product, idx) => (
              <motion.div key={idx} {...fadeInUp} className="group p-8 rounded-[3rem] bg-slate-50 border border-slate-100 transition-all hover:bg-white hover:shadow-2xl text-left flex flex-col h-full">
                ...
                <p className="text-slate-500 mb-6 font-light leading-relaxed flex-grow">{product.desc}</p>
                <div className="bg-white border border-slate-100 rounded-2xl p-4 mb-8">
                  <span className="text-[10px] font-black text-blue-600 uppercase tracking-widest block mb-2 font-sans">Best suited for:</span>
                  <p className="text-sm font-bold text-slate-700 font-sans">{product.suits}</p>
                </div>
                <Link to={product.path} className="w-full py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-3 hover:bg-blue-600 transition-all font-sans">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
              {[
                ...
                <div key={i} className="space-y-4">
                  <div className="w-12 h-12 rounded-xl bg-blue-600/20 flex items-center justify-center text-blue-400 border border-white/5">
                    {React.cloneElement(item.icon, { className: "w-6 h-6" })}
                  </div>
                  <h3 className="text-white font-bold text-lg leading-tight font-sans">{item.title}</h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 text-center">
            {[
              ...
              <div key={i} className="p-8 bg-slate-50 border border-slate-100 rounded-[2.5rem] flex flex-col items-center hover:bg-white transition-all shadow-sm">
                ...
                <h3 className="text-xs font-black uppercase tracking-widest text-slate-900 text-center font-sans">{item.title}</h3>
              </div>
            ))}
          </div>
          <p className="mt-10 text-center text-slate-500 italic font-light">Many customers use these diffusers as personal space scenting solutions, not just inside cars.</p>
        </section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {[
                  ...
                  <div key={i} className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-bold text-slate-700 font-sans">{benefit.title}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <section className="py-16 text-center font-sans border-t border-slate-100">
           ...
           <div className="flex flex-wrap justify-center gap-6">
             {[
               { label: "Desktop & Small Space Aroma Diffurers", path: "/desktop-and-small-space-aroma-diffusers/" },
               { label: "Commercial Aroma Diffurers", path: "/commercial-aroma-diffurers/" },
           ...
           <p className="mt-8 text-slate-400 text-sm text-center">These solutions help maintain freshness across all environments.</p>
        </section>
