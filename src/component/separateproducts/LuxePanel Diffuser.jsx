...
import { 
    SpeakerXMarkIcon, 
    SparklesIcon, 
    BoltIcon, // Re-added this to fix the 'not defined' error
    ArrowRightIcon,
    CheckCircleIcon,
    BeakerIcon,
    Battery100Icon,
...
const LuxePanelPage = () => {
  ...
  // --- SEO Implementation ---
  useEffect(() => {
    document.title = "LuxePanel Diffuser Dubai | Battery Aroma Diffuser";
    const metaDesc = document.querySelector('meta[name="description"]');
    if (metaDesc) {
      metaDesc.setAttribute("content", "LuxePanel Diffuser in Dubai is a battery-powered aroma diffuser for small spaces. Quiet, Bluetooth-controlled scenting solution available across UAE.");
    }
    ...
    canonical.setAttribute('href', 'https://www.coolmaxscent.com/wall-mounted-and-ceiling-aroma-diffusers/luxepanel-diffuser/');
  }, []);
    ...
    <div className="bg-[#FAF9F6] text-slate-900 font-sans selection:bg-blue-100 overflow-hidden text-left">
      ...
      <section className="py-16 bg-[#0c101b] text-white px-6 text-left">
        <div className="max-w-[1400px] mx-auto text-center text-left">
          <h2 className="text-2xl md:text-4xl font-serif mb-12 tracking-tight text-center text-white text-center">Key Highlights</h2>
        ...
        <div className="grid gap-16 text-left">
          <div className="grid md:grid-cols-2 gap-10 items-center text-left">
            <div className="order-2 md:order-1 text-left">
              <h3 className="text-xl font-serif mb-4 text-slate-900 text-left">Dual Power Flexibility</h3>
              ...
              <div className="space-y-3 text-left">
                <div className="flex items-center gap-3 p-3 bg-white border border-slate-100 rounded-xl shadow-sm text-left">
          ...
          <div className="grid md:grid-cols-2 gap-10 items-center text-left">
            ...
            <div className="order-2 text-left">
              <h3 className="text-xl font-serif mb-4 text-slate-900 text-left">Smart Control & Custom Scheduling</h3>
              <p className="text-sm text-slate-500 mb-4 font-light text-justify md:text-left text-left">The diffuser features a smart LCD display combined with Bluetooth remote control, allowing managers to:</p>
          ...
          <div className="grid md:grid-cols-2 gap-6 text-left">
            <div className="bg-slate-900 text-white p-8 rounded-[2rem] shadow-xl text-left">
               <BeakerIcon className="w-10 h-10 text-blue-400 mb-4 text-left" />
               <h3 className="text-xl font-serif mb-2 text-left text-white">Advanced Cold Air Diffusion</h3>
               <p className="text-slate-400 text-xs leading-relaxed mb-4 font-light text-left">Using two-fluid cold air atomization, LuxePanel atomizes fragrance finely and diffuses scent evenly without oil residue or overheating. </p>
               <div className="flex gap-4 text-[9px] font-black uppercase text-blue-200 tracking-widest text-left">
            ...
            <div className="bg-white border border-slate-100 p-8 rounded-[2rem] shadow-sm text-left">
               <SpeakerXMarkIcon className="w-10 h-10 text-blue-600 mb-4 text-left" />
               <h3 className="text-xl font-serif mb-2 text-slate-900 text-left">Quiet & Energy-Efficient</h3>
               <p className="text-slate-500 text-xs leading-relaxed mb-4 font-light italic text-justify md:text-left text-left">With noise levels below 35 dB, LuxePanel operates quietly, making it ideal for environments where silence and comfort are essential.</p>
               <div className="p-2 bg-slate-50 rounded-lg border border-slate-100 inline-block text-left">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Noise Level: ≤35 dB</p>
               </div>
            </div>
          </div>

          <div className="p-10 bg-white border border-slate-100 rounded-[2.5rem] shadow-sm flex flex-col items-center text-center text-left">
             <ArrowsPointingOutIcon className="w-12 h-12 text-blue-600 mb-6 text-left" />
             <h3 className="text-xl md:text-3xl font-serif mb-4 text-slate-900 text-center">Compact & Minimalist Design</h3>
             <p className="text-slate-500 max-w-xl mx-auto text-sm leading-relaxed font-light text-justify md:text-center text-center">LuxePanel Diffuser is designed with a slim, modern profile that blends effortlessly into professional interiors. Its compact size makes it suitable for wall-mounted or discreet placement without disrupting aesthetics.</p>
          </div>
        </div>
      </section>
          ...
          <div className="grid md:grid-cols-2 gap-6 text-left">
            <div className="bg-white rounded-[2rem] overflow-hidden border border-slate-200 shadow-sm text-left">
              <div className="bg-slate-900 p-4 text-white text-center">
                <h3 className="font-bold uppercase tracking-widest text-[10px] text-center">Standard Version (OV-5)</h3>
              </div>
              <table className="w-full border-collapse text-left text-left">
                <tbody className="text-xs text-left">
                  {[
                    ...
                    <tr key={i} className="border-b border-slate-100 hover:bg-slate-50 transition-colors text-left">
                      <td className="p-3 font-bold text-slate-400 uppercase tracking-widest text-[8px] text-left">{row[0]}</td>
                      <td className="p-3 text-slate-700 font-medium text-left">{row[1]}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="bg-white rounded-[2rem] overflow-hidden border border-slate-200 shadow-sm text-left">
              <div className="bg-blue-600 p-4 text-white text-center">
                <h3 className="font-bold uppercase tracking-widest text-[10px] text-center">Pro Version (OV-5 Pro)</h3>
              </div>
              <table className="w-full border-collapse text-left text-left">
                <tbody className="text-xs text-left">
                  {[
      ...
      <section className="py-16 px-6 lg:px-12 max-w-[1400px] mx-auto text-center text-left">
        <h2 className="text-2xl md:text-4xl font-serif mb-12 tracking-tight text-center text-slate-900 text-center">Ideal Applications</h2>
        <div className="flex flex-wrap justify-center gap-4 mb-16 text-center">
          {[
            ...
            <div key={i} className="px-6 py-4 bg-white border border-slate-100 rounded-2xl shadow-sm flex items-center gap-3 min-w-[180px] justify-center text-center">
        ...
        <div className="grid lg:grid-cols-2 gap-10 items-center text-left bg-slate-900 rounded-[3rem] p-8 md:p-16 text-white shadow-2xl text-left">
          <div className="relative h-80 text-left">
             <img src={LuxePanel4} alt="LuxePanel Installation" className="w-full h-full object-contain rounded-[2rem] shadow-xl text-left" />
             <div className="absolute -bottom-4 -right-4 p-6 bg-blue-600 text-white rounded-2xl shadow-xl text-left">
                <span className="text-xl font-black italic uppercase text-left">Hybrid Power</span>
                <p className="text-[8px] font-bold uppercase tracking-widest text-left">Battery + Type-C</p>
             </div>
          </div>
          <div className="text-left">
            <h2 className="text-2xl md:text-4xl font-serif mb-6 tracking-tight leading-tight text-left">Why Choose LuxePanel Diffuser</h2>
            <div className="space-y-3 text-left">
              {[
                ...
                <div key={i} className="flex items-center gap-4 p-3 bg-white/5 border border-white/10 rounded-xl text-left">
                  <CheckCircleIcon className="w-5 h-5 text-blue-400 flex-shrink-0 text-left" />
                  <span className="text-slate-200 text-xs font-medium text-left">{text}</span>
                </div>
              ))}
            </div>
            <p className="mt-6 text-slate-400 font-medium text-[11px] italic text-left text-justify">Dependable for businesses seeking a compact, battery-powered aroma diffuser.</p>
          </div>
        </div>
      </section>

      {/* ================= SECTION 7: COLLECTION ================= */}
      <section className="py-16 bg-slate-50 px-6 text-center text-left">
      ...
      <section className="py-20 px-6 text-center text-left">
        <div className="max-w-[1200px] mx-auto bg-[#0c101b] rounded-[3rem] p-10 md:p-16 text-center text-white relative overflow-hidden shadow-2xl text-center">
          ...
          <h2 className="text-3xl md:text-5xl font-serif mb-6 relative z-10 tracking-tight leading-tight text-center">Enhance Small Spaces with Smart Scenting</h2>
