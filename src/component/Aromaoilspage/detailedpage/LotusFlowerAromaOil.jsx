...
const LotusFlowerAromaOilDetail = () => {
  // Redirection Links
  const whatsappNumber = "+971509282702";
  const whatsappLink = `https://wa.me/${whatsappNumber}?text=Hello%20Cool%20Max%20Scent,%20I%20am%20interested%20in%20the%20Lotus%20Flower%20Aroma%20Oil%20(CM-0121-FH).`;
  ...
  useEffect(() => {
    ...
    canonical.setAttribute('href', 'https://www.coolmaxscent.com/fresh-aroma-oil/lotus-flower/');
  }, []);
  ...
  return (
    <div className="mt-16 bg-white text-slate-900 font-sans selection:bg-blue-100 overflow-hidden tracking-tight text-left">
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <motion.div {...fadeUp} className="order-2 lg:order-1">
            ...
            <h1 className="text-5xl md:text-7xl font-serif text-slate-950 mb-6 leading-[1.1] tracking-tighter uppercase ">
              LOTUS FLOWER <span className="italic font-light text-slate-600">AROMA OIL</span>
            </h1>
            <p className="text-xl text-blue-600 font-medium mb-4 uppercase tracking-wider font-sans">
              Soft Floral Fresh Fragrance for Calm & Elegant Environments
            </p>
            <div className="flex gap-6 mb-8 text-[11px] font-bold uppercase tracking-widest text-slate-400 font-sans">
                <span>Item Code: <span className="text-slate-900">CM-0121-FH</span></span>
            ...
            <p className="text-base md:text-lg text-slate-500 leading-relaxed font-light mb-8 font-sans">
              Lotus Flower Aroma Oil is a soft, elegant, and refreshing fragrance inspired by delicate florals and light fruity notes. This fragrance is designed to create a peaceful, graceful, and naturally uplifting atmosphere that feels clean and comforting.
            </p>
            <p className="text-base text-slate-500 leading-relaxed font-light mb-10 italic font-sans">
              It is ideal for spaces that focus on calmness, balance, and gentle freshness.
            </p>
      ...
      <section className="py-24 bg-slate-50 px-6 lg:px-12 text-center font-sans">
        <div className="max-w-[1200px] mx-auto text-center">
          ...
          <div className="grid md:grid-cols-3 gap-8 mb-16">
             {[
               { label: "Top Notes", notes: "Mandarin, Orange, Orange Peel, Bergamot, Fig Leaf, Pear, Apple", color: "bg-blue-400" },
               { label: "Middle Notes", notes: "White Lotus, Lotus Flower, Jasmine, Gardenia, Freesia", color: "bg-blue-500" },
               { label: "Base Notes", notes: "Vanilla, Patchouli, Musk, Cashmere, Iris", color: "bg-blue-600" }
             ].map((note, i) => (
          ...
          <p className="text-lg md:text-xl text-slate-600 max-w-4xl mx-auto leading-relaxed font-light italic text-center">
            "This fragrance opens with bright fruity freshness, moves into a soft floral heart, and settles into a smooth, musky base that feels warm and comforting."
          </p>
        </div>
      </section>
      ...
      <section className="py-24 bg-white px-6">
        <div className="max-w-[1200px] mx-auto grid lg:grid-cols-2 gap-16 items-center text-left font-sans">
           <div>
             ...
             <ul className="space-y-6">
                {[
                    "Fresh and uplifting opening",
                    "Elegant floral and natural character",
                    "Soft, smooth, long-lasting finish"
                ].map((text, i) => (
             ...
             <p className="mt-10 text-slate-500 leading-relaxed font-sans font-light text-left">
                Lotus Flower Aroma Oil creates a serene environment that feels welcoming without overwhelming the senses.
             </p>
           </div>
           <div className="grid grid-cols-2 gap-6 text-left">
              <div className="aspect-square bg-blue-50 rounded-[2.5rem] p-8 flex flex-col justify-end gap-3 group hover:bg-blue-600 transition-all duration-500 text-left">
              ...
              <div className="aspect-square bg-slate-950 rounded-[2.5rem] p-8 flex flex-col justify-end gap-3 group hover:bg-blue-700 transition-all duration-500 text-left">
                <HeartIcon className="w-12 h-12 text-blue-400 group-hover:text-white" />
                <span className="text-[11px] font-bold uppercase text-slate-200">Wellness Spas</span>
              </div>
              <div className="aspect-square bg-slate-100 rounded-[2.5rem] p-8 flex flex-col justify-end gap-3 group hover:bg-blue-600 transition-all duration-500 text-left">
                <SunIcon className="w-12 h-12 text-blue-600 group-hover:text-white" />
                <span className="text-[11px] font-bold uppercase text-slate-400 group-hover:text-blue-50">Yoga Spaces</span>
              </div>
              <div className="aspect-square bg-blue-100 rounded-[2.5rem] p-8 flex flex-col justify-end gap-3 group hover:bg-slate-950 transition-all duration-500 text-left">
                <CloudIcon className="w-12 h-12 text-blue-600 group-hover:text-white" />
                <span className="text-[11px] font-bold uppercase text-slate-500 group-hover:text-blue-50">Relaxation Areas</span>
              </div>
           </div>
        </div>
      </section>
      ...
      <section className="py-24 bg-slate-950 text-white px-6 font-sans text-left">
        <div className="max-w-[1200px] mx-auto text-center text-left">
            <h2 className="text-3xl md:text-5xl font-serif mb-20 text-center tracking-tight text-blue-400 italic font-light uppercase text-center">Why Choose Lotus Flower?</h2>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                {[
                    { title: "Balanced Diffusion", desc: "Medium strength for gentle, perfectly controlled aromatic presence." },
                    ...
                    <div key={i} className="p-10 rounded-[3rem] bg-white/5 border border-white/10 flex flex-col items-center gap-5 hover:bg-white/10 transition-colors text-left text-left">
                        ...
                        <h4 className="text-base font-bold uppercase tracking-widest text-white font-serif italic text-center text-left">{item.title}</h4>
                        <p className="text-sm text-slate-400 leading-relaxed font-sans font-light text-center text-left">{item.desc}</p>
                    </div>
                ))}
            </div>
        </div>
            ...
            <div className="bg-white rounded-[3rem] p-12 md:p-16 border border-slate-100 shadow-sm flex flex-col md:flex-row gap-12 items-center text-left">
                ...
                <div className="text-left">
                    ...
                    <p className="text-slate-500 font-light leading-relaxed text-left text-left">
                        Lotus Flower Aroma Oil is a professional fragrance oil intended for use with automated scent diffuser machines. Fragrance intensity can be adjusted based on space size and airflow to maintain optimal comfort.
                    </p>
                </div>
            </div>
        </div>
      ...
      <section className="py-24 px-6 text-center font-sans">
        <div className="max-w-[1200px] mx-auto bg-slate-950 rounded-[4rem] p-16 md:p-24 text-center text-white relative overflow-hidden shadow-2xl text-center">
          ...
          <h2 className="text-4xl md:text-7xl font-serif mb-10 relative z-10 tracking-tighter leading-tight uppercase text-center text-white text-center">Interested in <br/><span className="italic font-light text-slate-300 text-center">this fragrance?</span></h2>
          <p className="text-slate-400 text-lg md:text-2xl mb-12 relative z-10 max-w-3xl mx-auto leading-relaxed font-light text-center font-sans text-center">
          ...
          <div className="flex flex-wrap justify-center gap-6 relative z-10 font-sans text-center">
            <a href={whatsappLink} target="_blank" rel="noopener noreferrer" className="px-12 py-5 bg-white text-slate-900 rounded-full font-bold uppercase tracking-widest text-[11px] hover:bg-blue-600 hover:text-white transition-all shadow-xl flex items-center gap-3 font-sans text-center">
...
export default LotusFlowerAromaOilDetail;