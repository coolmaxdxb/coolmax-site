...
const NovaCarPage = () => {
  ...
  // --- SEO Implementation ---
  useEffect(() => {
    document.title = "NovaCar Diffuser | Smart Car Aroma Diffuser";
    const metaDesc = document.querySelector('meta[name="description"]');
    if (metaDesc) {
      metaDesc.setAttribute("content", "NovaCar Diffuser is a smart car aroma diffuser with automatic start-stop, premium metal body and efficient aromatherapy for vehicles.");
    }
    ...
    canonical.setAttribute('href', 'https://www.coolmaxscent.com/car-aroma-diffusers/nova-car-diffuser/');
  }, []);
    ...
    <div className="bg-[#FAF9F6] text-slate-900 font-sans selection:bg-blue-100 overflow-hidden text-left">
      ...
      <section className="py-16 bg-white px-6 text-left">
        <div className="max-w-[1000px] mx-auto text-center">
          <motion.div {...fadeIn}>
            ...
            <p className="text-base text-slate-600 leading-relaxed max-w-4xl mx-auto font-light text-center text-justify md:text-center">
      ...
      <section className="py-16 bg-[#0c101b] text-white px-6 text-left">
        <div className="max-w-[1400px] mx-auto text-center">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 text-left text-center">
            {[
      ...
      <section className="py-16 px-6 lg:px-12 max-w-[1400px] mx-auto text-left">
        <h2 className="text-2xl md:text-4xl font-serif mb-12 text-center tracking-tight text-center">Premium Metal Design & Build Quality</h2>
        
        <div className="grid gap-16 text-left">
          <div className="grid md:grid-cols-2 gap-10 items-center text-left">
            <div className="order-2 md:order-1 text-left">
              <h3 className="text-xl font-serif mb-4 text-slate-900 text-left">All-Metal Aviation Aluminum Body</h3>
              ...
              <h4 className="text-lg font-serif mb-3 text-left">Elegant Color Options</h4>
              <div className="flex flex-wrap gap-2 text-left">
                {["Black", "Silver", "Grey-Green", "Classic Silver", "Silver Chrome"].map((color, i) => (
                  <span key={i} className="px-3 py-1.5 bg-slate-100 rounded-full text-[10px] font-black uppercase text-slate-600 text-center">{color}</span>
                ))}
              </div>
            </div>
          ...
          <div className="grid md:grid-cols-2 gap-10 items-center text-left">
            ...
            <div className="text-left">
              <h3 className="text-xl font-serif mb-4 text-slate-900 text-left">Intelligent Diffusion Technology</h3>
              <div className="bg-slate-900 text-white p-6 rounded-2xl mb-4 text-left">
                ...
                <p className="text-slate-300 text-xs font-light leading-relaxed mb-4 text-left">Equipped with an intelligent induction system that automatically starts diffusion when the vehicle is in motion.</p>
                <div className="space-y-2 text-left">
                  {["Scent release only when needed", "Hands-free operation"].map((li, i) => (
                    <div key={i} className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-blue-200 text-left">
                      <CheckCircleIcon className="w-4 h-4 text-blue-500 flex-shrink-0" /> {li}
                    </div>
                  ))}
                </div>
              </div>
              <p className="text-slate-500 font-light text-xs italic text-left">Metal atomizer ensures fine dispersion without becoming overpowering.</p>
            </div>
          </div>

          <div className="grid md:grid-cols-3 gap-6 text-left">
            <div className="p-8 bg-white border border-slate-100 rounded-[2rem] shadow-sm text-left">
              <Battery100Icon className="w-8 h-8 text-blue-600 mb-4 text-left" />
              <h3 className="text-lg font-serif mb-2 text-left text-slate-900">Battery Performance</h3>
              <p className="text-slate-500 text-xs font-light text-left">2000mAh built-in battery delivering 12+ hours of usage from a single 3.5h charge.</p>
            </div>
            <div className="p-8 bg-blue-600 text-white rounded-[2rem] shadow-xl text-left">
              ...
              <h3 className="text-lg font-serif mb-2 text-white text-left">Quiet & Safe</h3>
              <p className="text-blue-100 text-xs font-light italic text-left">Noise level ≤40 dB ensures silent performance with a leak-proof stable build.</p>
            </div>
            <div className="p-8 bg-white border border-slate-100 rounded-[2rem] shadow-sm text-left">
              <BoltIcon className="w-8 h-8 text-blue-600 mb-4 text-left" />
              <h3 className="text-lg font-serif mb-2 text-left text-slate-900">Energy Efficient</h3>
              <p className="text-slate-500 text-xs font-light text-left">Operates at DC 5V / 2W, ensuring consistent fragrance output with low consumption.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 bg-slate-900 text-white px-6 text-left">
        <div className="max-w-[1200px] mx-auto grid lg:grid-cols-2 gap-12 items-center text-left">
          <div className="text-left">
            <h2 className="text-2xl md:text-4xl font-serif mb-6 tracking-tight text-blue-400 text-left">How NovaCar Works</h2>
            <p className="text-slate-400 text-sm leading-relaxed mb-8 font-light text-left text-justify">
          ...
          <div className="bg-white/5 p-10 rounded-[2rem] border border-white/10 flex flex-col items-center text-left">
             <BeakerIcon className="w-12 h-12 text-blue-500 mb-6 text-center" />
             <img src={NovaCar4} alt="Internal Mechanism" className="w-full h-40 object-contain rounded-xl opacity-80" />
             <p className="text-[10px] font-black uppercase mt-4 tracking-widest text-blue-200 text-center">Smart Sensing Atomization</p>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 px-6 lg:px-12 max-w-[1400px] mx-auto text-center text-left">
        <h2 className="text-2xl md:text-4xl font-serif mb-12 tracking-tight text-center">Ideal Applications</h2>
        <div className="flex flex-wrap justify-center gap-4 mb-10 text-center">
          {[
            ...
            <div key={i} className="px-6 py-4 bg-white border border-slate-100 rounded-2xl shadow-sm flex items-center gap-3 min-w-[200px] justify-center text-center">
                <item.icon className="w-5 h-5 text-blue-600" />
                <span className="text-[10px] font-bold text-slate-700 uppercase tracking-widest text-center">{item.label}</span>
            </div>
          ))}
        </div>
        <p className="text-sm text-slate-500 font-medium italic text-center">Specifically designed for automotive environments where controlled diffusion enhances comfort.</p>
      </section>

      {/* ================= SECTION 7: WHY CHOOSE ================= */}
      <section className="py-16 px-6 lg:px-12 max-w-[1400px] mx-auto text-left">
        <h2 className="text-2xl md:text-4xl font-serif mb-12 text-center tracking-tight text-center">Why Choose NovaCar?</h2>
      ...
      <section className="py-16 bg-slate-50 px-6 text-left">
        <div className="max-w-[1000px] mx-auto grid lg:grid-cols-2 gap-6 text-left">
          <div className="p-8 bg-white rounded-3xl shadow-sm border border-slate-100 text-left">
            ...
            <ul className="space-y-2 text-xs text-slate-500 font-light text-left text-left">
               <li className="text-left">• Easy fragrance oil replacement</li>
          ...
          <div className="p-8 bg-blue-600 text-white rounded-3xl shadow-xl text-left text-white">
            ...
            <ul className="space-y-2 text-xs text-blue-50 font-light text-left text-left">
               ...
               <li className="text-left">• Technical support & assistance</li>
            </ul>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 px-6 text-left">
        <div className="max-w-[1200px] mx-auto bg-[#0c101b] rounded-[4rem] p-10 md:p-16 text-center text-white relative overflow-hidden shadow-2xl text-left">
          ...
          <h2 className="text-3xl md:text-5xl font-serif mb-6 relative z-10 tracking-tight leading-tight text-center text-white">Get Started with NovaCar</h2>
