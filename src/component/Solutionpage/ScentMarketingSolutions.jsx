...
export default function ScentMarketingPage() {
  ...
  useEffect(() => {
    document.title = "Scent Marketing Solutions in UAE | Cool Max Scent";
    const metaDescription = document.querySelector('meta[name="description"]');
    if (metaDescription) {
      metaDescription.setAttribute("content", "Professional scent marketing solutions in UAE for hotels, offices, retail, & homes. Create memorable experiences using advanced scent systems by Cool Max Scent.");
    }
  }, []);
      ...
      <div className="relative z-10 max-w-[1600px] mx-auto px-6 md:px-16 text-left">
        ...
        <section className="min-h-[80vh] flex flex-col lg:flex-row items-center pt-24 lg:pt-12 pb-12 gap-12 text-left">
          ...
          <div className="w-full lg:w-1/2 flex flex-col items-start z-20 text-left">
            ...
            >
              <div className="inline-flex items-center gap-3 px-4 py-2 rounded-full bg-blue-50 border border-blue-100 mb-6 shadow-sm mt-16 relative z-10 text-left">
                ...
                <span className="text-blue-700 text-[10px] font-black uppercase tracking-[0.3em] text-left">
                  Scent Marketing Company in Dubai
                </span>
              </div>
              
              <h1 className="text-5xl md:text-7xl xl:text-6xl font-serif text-slate-900 tracking-tighter mb-6 leading-[0.95] text-left">
                Scent Marketing <br />
                <span className="italic text-slate-400 font-light pr-2 text-left">Solutions in UAE</span>
              </h1>
              
              <p className="text-lg md:text-xl text-slate-500 font-light max-w-xl mb-8 leading-relaxed text-left">
                Create Memorable Experiences with Professional Scent Systems UAE.
              </p>
              ...
              <div className="flex flex-wrap gap-4 text-left">
                ...
                <a 
                  ...
                  className="px-8 py-4 bg-[#25D366] text-white font-black text-[10px] uppercase tracking-widest rounded-2xl hover:bg-[#128C7E] transition-all shadow-xl flex items-center gap-3 text-left"
                >
          ...
          <div className="w-full lg:w-1/2 relative flex items-center justify-center text-left">
            ...
            >
              ...
              <img 
                src={HeroImg} 
                alt="Commercial Scent Marketing Solutions" 
                className="w-full h-auto object-contain relative z-10 drop-shadow-[0_20px_30px_rgba(0,0,0,0.15)] hover:scale-105 transition-transform duration-700 text-left" 
              />
            </motion.div>
          </div>
        </section>
          ...
          <div className="bg-slate-50 border border-slate-100 rounded-[3rem] p-8 md:p-16 overflow-hidden relative shadow-sm text-left">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center relative z-10 text-left">
              <motion.div 
                ...
                className="text-left"
              >
                <h2 className="text-3xl md:text-5xl font-serif text-slate-900 mb-6 tracking-tighter text-left">What Is Scent Marketing?</h2>
              ...
              >
                 <img src={FeatureImg1} alt="Hotel Scent Marketing UAE" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-1000 text-left" />
                 <div className="absolute inset-x-0 bottom-0 p-6 bg-gradient-to-t from-slate-900/80 to-transparent text-left">
                    <div className="bg-white/90 backdrop-blur-md p-4 rounded-xl flex items-center gap-4 shadow-xl text-left">
                       <div className="p-2 bg-blue-600 rounded-lg shadow-lg text-left">
                         <GlobeAsiaAustraliaIcon className="w-5 h-5 text-white" />
                       </div>
                       <p className="text-slate-900 font-serif text-base text-left">Enhancing atmospheres across the UAE.</p>
                    </div>
                 </div>
              </motion.div>
            </div>
          ...
          <div className="grid lg:grid-cols-12 gap-8 items-center text-left">
            <div className="lg:col-span-5 hidden lg:block text-left">
                <motion.div 
                    ...
                    whileInView={{ opacity: 1, x: 0 }}
                    viewport={{ once: true }}
                    className="relative h-[650px] rounded-[3rem] overflow-hidden border border-slate-100 shadow-xl text-left"
                >
                    <img src={FeatureImg2} alt="Professional Scent Systems UAE" className="w-full h-full object-cover text-left" />
                </motion.div>
            </div>

            <div className="lg:col-span-7 text-left">
                <div className="mb-8 text-left">
                    <h2 className="text-4xl md:text-6xl font-serif text-slate-900 mb-4 tracking-tighter leading-none text-left">Why Scent Marketing Matters for Businesses</h2>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-left">
                    ...
                    <motion.div 
                        key={idx}
                        className="group p-6 bg-slate-50 border border-slate-100 hover:border-blue-200 rounded-[2rem] transition-all duration-300 shadow-sm text-left"
                    >
                        <div className="w-8 h-1 bg-blue-600 rounded-full mb-4 group-hover:w-full transition-all duration-500 text-left" />
        ...
        <section className="py-16 border-t border-slate-100 text-left">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 text-left">
            {[
              ...
              >
                <div className="w-12 h-12 bg-white text-blue-600 flex items-center justify-center rounded-xl mb-6 shadow-sm border border-slate-100 group-hover:bg-blue-600 group-hover:text-white transition-all text-left">
                  {React.cloneElement(sol.icon, { className: "w-6 h-6 text-left" })}
                </div>
                <h3 className="text-base font-black uppercase tracking-tight text-slate-900 mb-2 text-left">{sol.title}</h3>
        ...
        <section className="py-16 text-left">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-5xl font-serif text-slate-900 mb-4 tracking-tighter leading-none text-center">Types of Spaces We Serve with Scent Marketing Solutions in UAE</h2>
            <p className="text-slate-500 max-w-2xl mx-auto text-center">Our Commercial Scent Marketing Solutions are designed for a wide range of environments across the UAE:</p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 text-left">
            {[
              { icon: <BuildingOfficeIcon />, title: "Hotels & Hospitality", desc: "We provide premium Hotel Scent Marketing UAE services that enhance guest experience in lobbies, corridors, rooms, and common areas." },
              { icon: <BuildingOffice2Icon />, title: "Offices & Corporate Spaces", desc: "Our Commercial Scent Marketing Solutions help create a professional and comfortable work environment that supports focus and productivity." },
              ...
              <div key={i} className="p-8 bg-slate-50 border border-slate-100 rounded-[2.5rem] flex flex-col items-center hover:bg-white transition-all shadow-sm text-left">
                ...
                <h3 className="text-xs font-black uppercase tracking-widest text-slate-900 mb-2 text-center">{space.title}</h3>
                <p className="text-[10px] text-slate-500 leading-relaxed font-bold text-center">{space.desc}</p>
              </div>
            ))}
          </div>
        </section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 text-left">
                {[
                  ...
                  <div key={i} className="flex items-center gap-3 text-left">
                    <CheckCircleIcon className="w-5 h-5 text-blue-400 text-left" />
                    <span className="text-white text-sm font-medium text-left">{tech}</span>
                  </div>
                ))}
              </div>
              <p className="mt-10 text-slate-400 italic text-sm text-left">From small home scent diffusers to large HVAC scent diffuser installations, our systems are designed for long-term use and easy maintenance.</p>
           </div>
        </section>
          ...
          <div className="max-w-5xl mx-auto space-y-4 text-left">
            {[
              ...
              >
                ...
                <ArrowRightIcon className="w-6 h-6 text-slate-300 group-hover:translate-x-2 group-hover:text-blue-600 transition-all hidden md:block text-left" />
              </motion.div>
            ))}
          </div>
        </section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
              {[
                ...
                <div key={i} className="flex items-center gap-4 text-left">
                  ...
                  <span className="text-slate-700 font-medium text-left">{reason}</span>
                </div>
              ))}
            </div>
            <p className="mt-10 text-slate-500 font-serif italic text-lg text-left">We aim to build long-term partnerships by delivering Commercial Scent Marketing Solutions that actually work in everyday environments.</p>
          </div>
        </section>
